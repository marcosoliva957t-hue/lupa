...
def consultar_cnpj_multi_api(cnpj_limpo):
    print(f"{Y}[*] Coletando dados da Receita Federal (Multi-API Fallback)...{W}")
    
    # 1. BrasilAPI
    url1 = f"https://brasilapi.com.br/api/cnpj/v1/{cnpj_limpo}"
    data = make_http_request(url1)
    if data and 'razao_social' in data:
        print(f"  {G}[+] Dados obtidos via BrasilAPI{W}")
        return data, 'BrasilAPI'
        
    # 2. MinhaReceita
    url2 = f"https://minhareceita.org/{cnpj_limpo}"
    data = make_http_request(url2)
    if data and 'razao_social' in data:
        print(f"  {G}[+] Dados obtidos via MinhaReceita{W}")
        if 'qsa' not in data and 'socios' in data:
            data['qsa'] = data['socios']
        return data, 'MinhaReceita'

    # 3. CNPJ.ws (pública)
    url3 = f"https://publica.cnpj.ws/cnpj/{cnpj_limpo}"
    data_raw = make_http_request(url3)
    if data_raw and 'estabelecimento' in data_raw:
        print(f"  {G}[+] Dados obtidos via CNPJ.ws{W}")
        est = data_raw.get('estabelecimento', {})
        data = {
            'cnpj': cnpj_limpo,
            ...
            'capital_social': float(data_raw.get('capital_social', 0)),
            'email': est.get('email', ''),
            ...
            'qsa': [{'nome_socio': s.get('nome'), 'qualificacao_socio': s.get('qualificacao_socio', {}).get('descricao'), 'faixa_etaria': ''} for s in data_raw.get('socios', [])]
        }
        return data, 'CNPJ.ws'
...
def gerar_dorks_expandidas(razao_social, cnpj, dominio=None):
    print(f"\n{C}[!] GOOGLE DORKS ESTRATÉGICAS DE INTELIGÊNCIA (LUPA STYLE):{W}")
    
    print(f"\n  {B}1. Vazamentos, Documentos e Credenciais:{W}")
    print(f"    -> Documentos PDF/DOC : ext:pdf OR ext:doc OR ext:xls \"{cnpj}\" OR \"{razao_social}\"")
    print(f"    -> Senhas e Pastes    : site:pastebin.com OR site:github.com \"{cnpj}\"")
    if dominio:
        print(f"    -> Domínio Leaks      : site:pastebin.com OR site:github.com \"{dominio}\"")
    print(f"    -> Configs/SQL/Env    : ext:env OR ext:sql OR ext:log OR ext:bkp \"{razao_social}\"")

    print(f"\n  {B}2. Processos Judiciais, Trabalhistas e Débitos:{W}")
    print(f"    -> JusBrasil          : site:jusbrasil.com.br \"{cnpj}\" OR \"{razao_social}\"")
    print(f"    -> Escavador          : site:escavador.com \"{cnpj}\"")
    print(f"    -> Tribunais PJe      : site:pje.jus.br OR site:tjsp.jus.br \"{razao_social}\"")
    print(f"    -> Dívida PGFN        : site:regularize.pgfn.gov.br \"{cnpj}\"")

    print(f"\n  {B}3. Diários Oficiais, Licitações, Sintegra e Cartórios:{W}")
    print(f"    -> Diário Oficial União: site:in.gov.br \"{cnpj}\" OR \"{razao_social}\"")
    print(f"    -> Licitações PNCP     : site:pncp.gov.br OR site:comprasnet.gov.br \"{cnpj}\"")
    print(f"    -> Protestos Cartório  : site:cenprot.org.br OR site:protestosp.com.br \"{razao_social}\"")
    print(f"    -> Sintegra/IE         : \"Inscrição Estadual\" OR \"Sintegra\" \"{cnpj}\"")

    print(f"\n  {B}4. Reputação, Consumidor e Mídia:{W}")
    print(f"    -> Reclame Aqui       : site:reclameaqui.com.br \"{razao_social}\"")
    print(f"    -> Fraude e Mídia     : intitle:\"investigação\" OR intitle:\"fraude\" OR intitle:\"operacao\" \"{razao_social}\"")
